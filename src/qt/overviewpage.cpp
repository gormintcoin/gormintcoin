...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizegormintAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizegormintAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizegormintAmount = strAnonymizegormintAmount.remove(strAnonymizegormintAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizegormintAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizegormintAmount * COIN) nMaxToAnonymize = nAnonymizegormintAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizegormintAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizegormintAmount));
        strAnonymizegormintAmount = strAnonymizegormintAmount.remove(strAnonymizegormintAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizegormintAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
