...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a gormint address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no gormint: URI
...
{
    // Convert gormint:// to gormint:
    //
    //    Cannot handle this later, because gormint:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open gormint.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "gormint.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for gormint.lnk
...
{
    return GetAutostartDir() / "gormint.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a gormint.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=gormint\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the gormint app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add gormint app to startup item list
