Contributing to gormint Core
============================

The gormint Core project operates an open contributor model where anyone is
...
The following applies to code changes to the gormint Core project, and is not to be
confused with overall gormint Network Protocol consensus changes.

Whether a pull request is merged into gormint Core rests with the project merge
...
Patches that change gormint consensus rules are considerably more involved than
...
Where a patch set proposes to change the gormint consensus, it must have been
...
The project leader is the release manager for each gormint Core release.
